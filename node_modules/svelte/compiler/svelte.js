(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function transitions$1(validator, prop) {
        if (prop.value.type !== 'ObjectExpression') {
            validator.error(prop, {
                code: `invalid-transitions-property`,
                message: `The 'transitions' property must be an object literal`
    ...
    function validateEventHandlerCallee(validator, attribute, refCallees) {
        ...
        const validCallees = ['this.*', 'refs.*', 'event.*', 'options.*', 'console.*'].concat(Array.from(validBuiltins), Array.from(validator.methods.keys()));
    ...
    class AwaitBlock extends Node$1 {
        ...
        init(block, stripWhitespace, nextSibling) {
            ...
            ['pending', 'then', 'catch'].forEach(status => {
                ...
                if (child.block.hasIntros)
                    hasIntros = true;
                if (child.block.hasOutros)
                    hasOutros = true;
            });
    ...
    class Component extends Node$1 {
        ...
        build(block, parentNode, parentNodes) {
            ...
            const attributeObject = usesSpread
                ? '{}'
                : stringifyProps(this.attributes.map(attr => `${quoteNameIfNecessary(attr.name)}: ${attr.getValue()}`));
            ...
            if (this.compiler.options.nestedTransitions) {
                block.builders.outro.addLine(`${name}._fragment.o(#outrocallback);`);
            }
        }
        remount(name) {
        ...
        build(block, parentNode, parentNodes) {
            ...
            if (!this.namespace && this.canUseInnerHTML && this.children.length > 0) {
                if (this.children.length === 1 && this.children[0].type === 'Text') {
                    block.builders.create.addLine(`${node}.textContent = ${stringify(this.children[0].data)};`);
                }
                else {
                    block.builders.create.addLine(`${node}.innerHTML = ${stringify(this.children.map(toHTML).join(''))};`);
                }
            }
            else {
                this.children.forEach((child) => {
            ...
            if (this.compiler.options.dev) {
                ...
                block.builders.hydrate.addLine(`@addLoc(${this.var}, ${this.compiler.fileVar}, ${loc.line}, ${loc.column}, ${this.start});`);
            }
        }
        addBindings(block) {
    ...
    class IfBlock extends Node$1 {
        ...
        init(block, stripWhitespace, nextSibling) {
            ...
            function attachBlocks(node) {
                ...
                if (node.block.hasIntros)
                    hasIntros = true;
                if (node.block.hasOutros)
                    hasOutros = true;
                if (isElseIf(node.else)) {
                    ...
                    if (node.else.block.hasIntros)
                        hasIntros = true;
                    if (node.else.block.hasOutros)
                        hasOutros = true;
                }
            }
            attachBlocks(this);
        ...
        build(block, parentNode, parentNodes) {
            ...
            if (this.else) {
                if (hasOutros) {
                    ...
                    if (this.compiler.options.nestedTransitions) {
                        block.builders.outro.addLine(`${name}.o(#outrocallback);`);
                    }
                }
                else {
                ...
                if (defaultExport) {
                    ...
                    if (templateProperties.animations) {
                        templateProperties.animations.value.properties.forEach((property) => {
                            addDeclaration(getMethodName(property.key), property.value, false, 'animations');
                        });
                    }
	...
	`);
        ...
        return compiler.generate(result, options, {
            banner: `/* ${compiler.file ? `${compiler.file} ` : ``}generated by Svelte v${"2.11.0"} */`,
            sharedPath,
            name,
            format,
        });
